    ...
    enum ibv_access_flags permission) 
{
	...
	if (!pd) {
		LOG_ERROR("Protection domain is NULL \n");
		return NULL;
	}
	void *buf = calloc(1, size);
	if (!buf) {
		LOG_ERROR("failed to allocate buffer, -ENOMEM\n");
		return NULL;
	}
	LOG_DEBUG("Buffer allocated: %p , len: %u \n", buf, size);
		...
		enum ibv_access_flags permission)
{
	struct ibv_mr *mr = NULL;
	if (!pd) {
		LOG_ERROR("Protection domain is NULL, ignoring \n");
		return NULL;
	}
	mr = ibv_reg_mr(pd, addr, length, permission);
	if (!mr) {
		LOG_ERROR("Failed to create mr on buffer, errno: %d \n", -errno);
		return NULL;
	}
	LOG_DEBUG("Registered: %p , len: %u , stag: 0x%x \n", 
			mr->addr, 
...
{
	if (!mr) {
		LOG_ERROR("Passed memory region is NULL, ignoring\n");
		return ;
	}
	void *to_free = mr->addr;
	rdma_buffer_deregister(mr);
	LOG_DEBUG("Buffer %p free'ed\n", to_free);
	free(to_free);
}
...
void rdma_buffer_deregister(struct ibv_mr *mr) 
{
	...
	LOG_DEBUG("Deregistered: %p , len: %u , stag : 0x%x \n", 
			mr->addr, 
			(unsigned int) mr->length, 
			mr->lkey);
	ibv_dereg_mr(mr);
		...
		struct rdma_cm_event **cm_event)
{
	...
	if (ret) {
		LOG_ERROR("Failed to retrieve a cm event, errno: %d \n",
				-errno);
		return -errno;
	}
	/* lets see, if it was a good event */
	if(0 != (*cm_event)->status){
		LOG_ERROR("CM event has non zero status: %d\n", (*cm_event)->status);
		ret = -((*cm_event)->status);
	...
	LOG_DEBUG("A new %s type event is received \n", rdma_event_str((*cm_event)->event));
		...
		struct ibv_wc *wc, int max_wc)
{
	...
	ret = ibv_get_cq_event(comp_channel, /* IO channel where we are expecting the notification */ 
       ...
       if (ret) {
	       LOG_ERROR("Failed to get next CQ event due to %d \n", -errno);
	       return -errno;
       }
       /* Request for more notifications. */
       ret = ibv_req_notify_cq(cq_ptr, 0);
       if (ret){
	       LOG_ERROR("Failed to request further notifications %d \n", -errno);
	       return -errno;
       }
	...
	*/
       ...
       do {
	       ...
	       if (ret < 0) {
		       LOG_ERROR("Failed to poll cq for wc due to %d \n", ret);
		       /* ret is errno here */
		       return ret;
	       }
	       total_wc += ret;
       } while (total_wc < max_wc); 
       LOG_DEBUG("%d WC are completed \n", total_wc);
