...
static int aeApiCreate(aeEventLoop *eventLoop)
{
    ...
    aeEpollState *state = (aeEpollState *)malloc(sizeof(aeEpollState));
 ...
 * 获取可执行事件
 */
static int aeEpoll(aeEventLoop *eventLoop, struct timeval *tvp)
{
    aeEpollState *state = (aeEpollState *)eventLoop->epoll_state;
                      ...
                      aeFileProc *proc, void *clientData)
{
    ...
    // 监听指定 fd 的指定事件
    if (aeEpollAddEvent(eventLoop, fd, mask) == -1)
        return AE_ERR;
...
int aeProcessEvents(aeEventLoop *eventLoop, int flags)
{
    ...
    // 2.处理完就绪的网络事件后，执行时间事件（因为时间时间相对来说没有网络事件时延要求高，所以后面执行）
