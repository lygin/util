...
//pool指向的是整个共享内存空间的起始地址   slab结构是配合共享内存使用的 可以以limit req模块为例，参考ngx_http_limit_req_module
...
{ //这儿假设page_size是4KB  
    ...
    if (page->next != page) { //如果之前已经有此类大小obj且那个已经分配的内存缓存页还未满
        ...
        if (shift < ngx_slab_exact_shift) { //小obj，size < 128B，更新内存缓存页中的bitmap，并返回待分配的空间在缓存的位置  
            ...
            do {
                if (page->slab != NGX_SLAB_BUSY) { //该page没用完
...
*/
void
ngx_slab_free_unlocked(ngx_slab_pool_t *pool, void *p)
{
    size_t            size;
