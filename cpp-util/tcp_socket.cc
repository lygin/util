...
TcpSocket::~TcpSocket() {
  if (fd_ != -1) {
    int ret = close(fd_);
    if (ret < 0) {
      LOG_FATAL("close failed fd %d errno [%s]", fd_, strerror(errno));
    }
  }
}
...
void TcpSocket::Shutdown() {
  ...
  if (ret < 0) {
    LOG_FATAL("shutdown failed fd %d errno [%s]", fd_, strerror(errno));
  }
}
...
int TcpSocket::InitSocket(bool non_blocked) {
  ...
  if (fd_ == -1) {
    ...
    if (fd_ == -1) {
      LOG_FATAL("socket failed fd %d errno [%s]", fd_, strerror(errno));
  ...
  if (ret == -1) {
    LOG_FATAL("setsockopt failed fd %d errno [%s]", fd_,
                      strerror(errno));
    return kNetWorkError;
  }
...
int TcpSocket::Bind(const InetAddr& local_addr) {
  ...
  if (ret == -1) {
    LOG_FATAL("bind failed fd_ %d addr %s errno [%s]", fd_,
...
int TcpSocket::Connect(const InetAddr& peer_addr) {
  ...
  if (ret == -1) {
    ...
    LOG_FATAL("connect failed fd_ %d local %s peer %s errno [%s]", fd_,
...
int TcpSocket::Listen() {
  ...
  if (ret == -1) {
    LOG_FATAL("listen failed fd_ %d local %s errno [%s]", fd_,
...
int TcpSocket::Accept(InetAddr& accepted_addr) {
  ...
  while (1) {
    ...
    if (accepted_fd == -1) {
      ...
      LOG_FATAL("accept failed fd_ %d local %s errno [%s]", fd_,
                        local_addr_.ToString().c_str(), strerror(errno));
      return kNetWorkError;
    } else {
...
int TcpSocket::SetNonBlock(bool non_blocked) {
  ...
  if (flag == -1) {
    LOG_FATAL("fcntl failed errno [%s]", strerror(errno));
    return kNetWorkError;
  }
...
int TcpSocket::CheckIfValid() {
  ...
  if (ret == -1) {
    LOG_FATAL("getsockopt failed errno [%s]", strerror(errno));
    return kNetWorkError;
  }

  if (error != 0) {
    LOG_FATAL("getsockopt succ errno [%s]", strerror(errno));
    return kNetWorkError;
  }
