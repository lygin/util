...
struct ngx_slab_page_s {
    //会使用位图来标记page页面中的slot块是否使用.并将位图存储在slab中.slab是32为整形变量,即对应32块slot.
...
*/
    ...
    ngx_slab_page_t   free; // 是一个链表头,用于连接空闲页面.free->next指向pages * sizeof(ngx_slab_page_t) 
...
//参数size就是需要分配的内存大小，返回值就是内存块的首地址
//无锁版本
